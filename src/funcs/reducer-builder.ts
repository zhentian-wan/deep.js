...
export class DynamicReducer<TState, TBuilder extends Record<string, any> = {}> {
  private handlers = {} as Record<
    string,
    (state: TState, action: HandlersToDiscriminatedUnion<TBuilder>) => TState
  >;

  addHandler<TType extends string, TPayload>(
    type: TType,
    handler: (state: TState, payload: TPayload) => TState
  ): DynamicReducer<TState, TBuilder & Record<TType, TPayload>> {
  ...
  reduce(
    state: TState,
    action: HandlersToDiscriminatedUnion<TBuilder>
  ): TState {
